...
import { Type, Palette, Trash2, Copy, Layers, AlignLeft, AlignCenter, AlignRight, Bold, Italic, Underline, Settings, Info, Clock } from 'lucide-react';
...
type TabType = 'properties' | 'info' | 'history' | 'layers';
type PanelTab = 'properties' | 'layers' | 'history' | 'info';
...
export function PropertiesPanel() {
    ...
    // Tab navigation
    const tabs = [
        { id: 'properties' as TabType, icon: Settings, label: 'Properties' },
        { id: 'layers' as TabType, icon: Layers, label: 'Layers' },
        { id: 'history' as TabType, icon: Clock, label: 'History' },
        { id: 'info' as TabType, icon: Info, label: 'Info' },
    ];

    return (
        <aside className="w-80 bg-white dark:bg-slate-900 border-l border-slate-200 dark:border-slate-700 shrink-0 flex flex-col">
            {/* Tab Navigation */}
            ...
            <div className="flex-1 overflow-y-auto">
                {activeTab === 'properties' && selectedAnnotation && (
                    <div className="p-4">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="pt-4 border-t border-slate-100 space-y-2">
                                ...
                                <Button
                                    variant="ghost"
                                    className="w-full text-rose-500 hover:text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-900/20 justify-start gap-2"
                ...
                {activeTab === 'properties' && !selectedAnnotation && (
                    <div className="p-4 text-center text-slate-400 dark:text-slate-500 mt-8">
                        <Palette className="w-12 h-12 mx-auto mb-2 opacity-50" />
                        <p className="text-sm">Select an annotation to edit properties</p>
                    </div>
                )}

                {activeTab === 'info' && <DocumentInfo />}
                {activeTab === 'history' && <EditHistory />}
                {activeTab === 'layers' && <DocumentLayers />}
            </div>
        </aside>
    );
}
